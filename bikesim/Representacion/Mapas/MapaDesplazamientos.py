...
class MapaDesplazamientos:
    ...
    def representarInstante(self, instante, accion=1, tipo=True):
        ...
        if not matrizDesp.empty:
            ...
            mapa = folium.Map(location=[Constantes.COORDENADAS[round(Constantes.COORDENADAS.shape[0] / 2), 1],
                                        Constantes.COORDENADAS[round(Constantes.COORDENADAS.shape[0] / 2), 2]],
                              zoom_start=12)
    ...
    def generar_mapa(self, lista_coordenadas, modo_conduccion, listaId,
                     API_KEY='5b3ce3597851110001cf62484732354d82964f0491b986aeb994386d', listaPeso=[]):
        ...
        for coord in lista_coordenadas:
            ...
            folium.Marker(location=coordinates[0],
                          popup=f'Start of Route {i} ' + 'Station ' + str(listaId[index][0])).add_to(m)
            folium.Marker(location=coordinates[-1], icon=folium.Icon(color='red'),
                          popup=f'End of route {i} ' + 'Station ' + str(listaId[index][1])).add_to(m)
