...
export default function GraphAnalysisSidebar({runId}: GraphAnalysisSidebarProps) {
    ...
    const handleAnalyze = async () => {
        ...
        const requests = selectedCharts.map(async key => {
            ...
            if (arg == null) {
                throw new Error(`Parámetros inválidos para la gráfica ${key}`);
            }

            const payload: any = {...commonPayload};

            if (key === 'graf_barras_est_med') {
                payload.graf_barras_est_med = arg;
            } else if (key === 'graf_barras_est_acum') {
                payload.graf_barras_est_acum = arg;
            } else if (key === 'graf_barras_dia') {
                payload.graf_barras_dia = arg;
            } else if (key === 'graf_linea_comp_est') {
                payload.graf_linea_comp_est = arg;
            } else if (key === 'graf_linea_comp_mats') {
                payload.graf_linea_comp_mats = arg;
            }
