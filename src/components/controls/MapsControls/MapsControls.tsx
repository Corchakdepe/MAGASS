...
function normalizeStations(value: string): string {
  return value
    .split(";")
    .map((v) => v.trim())
    .filter((v) => /^\d+$/.test(v))
...
export function MapsControls({
  ...
  const setStationsFor = (key: MapKey, next: string) => {
    setStationsMaps((prev) => ({ ...prev, [key]: normalizeStations(next) }));
  };
        ...
        <div className="space-y-1.5">
          ...
          <Popover open={mapPickerOpen} onOpenChange={setMapPickerOpen}>
            <PopoverTrigger asChild>
              ...
              >
                <span className="truncate flex-1 text-left font-medium">{selectedMapLabel}</span>
                    ...
                    {MAPAS.map((m) => {
                      ...
                      return (
                        <CommandItem
                          ...
                          value={m.label}
                          onSelect={() => {
                            setSelectedMaps([m.arg]);
                            if (
                              m.arg === "mapa_densidad" ||
                              m.arg === "mapa_voronoi" ||
                              m.arg === "mapa_circulo"
                            ) {
                              onActiveStationsTargetKeyChange?.(m.arg);
                            }
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setInstantesFor("mapa_desplazamientos_inst", normalizeNumber(e.target.value))
                  }
                />
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setInstantesFor("mapa_desplazamientos_d_ori", normalizeNumber(e.target.value))
                    }
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setInstantesFor("mapa_desplazamientos_d_dst", normalizeNumber(e.target.value))
                    }
                  />
                </div>
              </div>
    ...
    <div className="space-y-1.5">
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            <span className="truncate flex-1 text-left font-medium">{selectedLabel}</span>
            <ChevronsUpDown className="ml-2 h-3.5 w-3.5 shrink-0 text-text-tertiary" />
          </Button>
        </PopoverTrigger>
        <PopoverContent
