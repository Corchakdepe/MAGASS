...
export class SimulationAPIClient {
    ...
    async getRun(runId: string): Promise<BikeSimulationContext> {
        ...
        const context: BikeSimulationContext = {
            meta: {
                runId,
                simName: initialData?.simName || initialData?.simname || initialData?.name || runId || 'Unnamed', // FIX: Use initialData
...
private parseSimulationResponse(
  ...
  const runId = data.folder || data.simfolder || data.output_folder_name || '';
  ...
  return {
    meta: {
      runId,
      simName: data?.simName || data?.simname || data?.name || runId || 'Unnamed',
