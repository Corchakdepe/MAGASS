...
def analizarCMD(comando: [str]):
    ...
    if histograma_medio_estacion != Constantes.CARACTER_NULO_CMD:
        ...
        write_series_csv(join(Constantes.RUTA_SALIDA, titulo),
                         x=x,
                         ys={"mean": serie},
                         meta={"type": "bar", "title": "Media Estación " + aux_cadena[0], "xLabel": "Hora", "yLabel": "Media"})
    ...
    if histograma_acumulado_estacion != Constantes.CARACTER_NULO_CMD:
        ...
        write_series_csv(join(Constantes.RUTA_SALIDA, titulo),
                         x=x,
                         ys={"cum": values},
                         meta={"type": "line", "title": "Acumulado Estación " + aux_cadena[0], "xLabel": "Hora", "yLabel": "Acumulado"})
    ...
    if histograma_dia != Constantes.CARACTER_NULO_CMD:
        ...
        vals = matrizDeseada.iloc[slice_idx, :].mean(axis=0).tolist() if media else matrizDeseada.iloc[slice_idx, :].sum(axis=0).tolist()
        write_series_csv(join(Constantes.RUTA_SALIDA, titulo),
                         x=x,
                         ys={"value": vals},
                         meta={"type": "line", "title": "Días " + str(dias), "xLabel": "Hora", "yLabel": "Valor", "freq": frecuencia, "media": media})
    ...
    if histograma_comparar_estaciones != Constantes.CARACTER_NULO_CMD:
        ...
        x = _hour_index(matrizDeseada.shape[1])
        ys = {}
        for est, dias in zip(estaciones, diasComparacion):
            idx = [h + d*24 for d in dias for h in range(24)]
            ys[f"station_{est}"] = matrizDeseada.iloc[idx, :].mean(axis=0).tolist()
        write_series_csv(join(Constantes.RUTA_SALIDA, titulo),
                         x=x, ys=ys,
                         meta={"type": "line", "title": "Comparación estaciones", "xLabel": "Hora", "yLabel": "Media"})
    ...
    if histograma_comparar_matrices != Constantes.CARACTER_NULO_CMD:
        if Constantes.MATRIZ_CUSTOM != None:
            ...
            write_series_csv(join(Constantes.RUTA_SALIDA, titulo),
                             x=x, ys=ys,
                             meta={"type": "line", "title": "Comparar matrices", "xLabel": "Hora", "yLabel": "Media", "delta": deltaMatriz, "media": media})
