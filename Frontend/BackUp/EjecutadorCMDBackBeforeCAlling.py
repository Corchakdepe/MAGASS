...
def run_simulation(
    ...
    """
    High-level simulation entry-point used by FastAPI.
    Keeps compatibility with simularCMD by building the comando list.
    """
    if dias is None:
        dias_arg = Constantes.CARACTER_NULO_CMD
    else:
        dias_arg = dias
...
def run_analysis(
    ...
    delta_media: str | None = None,
    delta_acumulada: str | None = None,
    graf_barras_est_med: str | None = None,
    graf_barras_est_acum: str | None = None,
    graf_barras_dia: str | None = None,
    graf_linea_comp_est: str | None = None,
    graf_linea_comp_mats: str | None = None,
    mapa_densidad: str | None = None,
    video_densidad: str | None = None,
    mapa_voronoi: str | None = None,
    mapa_circulo: str | None = None,
    mapa_desplazamientos: str | None = None,
    filtrado_EstValor: str | None = None,
    filtrado_EstValorDias: str | None = None,
    filtrado_Horas: str | None = None,
    filtrado_PorcentajeEstaciones: str | None = None,
):
    N = Constantes.CARACTER_NULO_CMD
    ...
    comando = [
        ...
        path_entrada,
        path_salida,
        seleccion_agregacion,
        delta_media or N,
        delta_acumulada or N,
        graf_barras_est_med or N,
        graf_barras_est_acum or N,
        graf_barras_dia or N,
        graf_linea_comp_est or N,
        graf_linea_comp_mats or N,
        mapa_densidad or N,
        video_densidad or N,
        mapa_voronoi or N,
        mapa_circulo or N,
        mapa_desplazamientos or N,
        filtrado_EstValor or N,
        filtrado_EstValorDias or N,
        filtrado_Horas or N,
        filtrado_PorcentajeEstaciones or N,
    ]

    return analizarCMD(comando)
...
def run_simulador_estadistico(
    ...
    """
    Wrapper for simuladorEstadistico using explicit params.
    """
    comando = [
        "",
...
def run_restar_directorios(
    ...
    """
    Wrapper for restarDirectorios using explicit params.
    """
    comando = [
        "",
        "restar_directorios",
        ruta_directorio1,
        ruta_directorio2,
        ruta_directorio_salida,
    ]
    restarDirectorios(comando)
...
def write_series_csv(path_png: str, x, ys: dict, meta: dict = None):
    """
    Write a wide CSV with an 'x' column and one column per series in ys,
    plus optional .meta.json to carry chart hints. This is additive to PNG output.
    """
...
def _hour_index(n_cols: int):
    # Generic x-axis for per-hour or per-slot series
    return list(range(n_cols))


# -------------------------
# Existing functions + CSV
# -------------------------
...
def simularCMD(comando: [str]):
    ...
    if stress > 0:
        ...
        Extractor.extraerStressAplicado(ficheros[0], ficheroDelta_salidaStress, stress, tipoStress=int(tipoStress),
        ...
        Extractor.extraerStressAplicado(ficheros[5], ficheroTendencias_salidaStress, stress, tipoStress=int(tipoStress),
                                        listaEstaciones=estaciones_stress)
...
def analizarCMD(comando: [str]):
    ...
    if len(comando) < 21:
     raise ValueError(f"analizarCMD necesita 21 argumentos; recibido {len(comando)}: {comando}")

    charts: list[dict] = []  # NEW: collect JSON chart specs here
    ...
    if histograma_dia != Constantes.CARACTER_NULO_CMD:
        ...
        # --- CSV that matches flags ---
        slice_idx = [h + d * 24 for d in dias for h in range(24)]
        # base series per column (hour)
        base_vals = matrizDeseada.iloc[slice_idx, :].mean(axis=0).to_numpy() if media \
            else matrizDeseada.iloc[slice_idx, :].sum(axis=0).to_numpy()

        if frecuencia:
            # compute histogram of these values
            bin_count = 20  # or same as in matplotlib
            ...
            # use bin centers as x and counts as y
            ...
            # no frequency: keep per-hour series like before
            x = _hour_index(matrizDeseada.shape[1])
    ...
    if histograma_comparar_matrices != Constantes.CARACTER_NULO_CMD:
        if Constantes.MATRIZ_CUSTOM != None:
            ...
            write_series_csv(join(Constantes.RUTA_SALIDA, titulo),
                             x=x, ys=ys,
                             meta={"type": "line", "title": "Comparar matrices", "xLabel": "Hora", "yLabel": "Media",
                                   "delta": deltaMatriz, "media": media})
            charts.append({
    ...
    if mapa_densidad_video != Constantes.CARACTER_NULO_CMD:
        ...
        print("Generar video con " + str(momentoInicio) + " " + str(momentoFinal) + " " + str(
            join(Constantes.RUTA_SALIDA, nombre)))
        mapa.realizarVideoHeatmap(momentoInicio, momentoFinal, rutaSalida=join(Constantes.RUTA_SALIDA, nombre))

        # NEW CSV tall slice for range
        rows = []
        ...
        df.to_csv(join(Constantes.RUTA_SALIDA, nombre.replace(".mp4", ".csv")), index=False)
    ...
    if mapa_voronoi != Constantes.CARACTER_NULO_CMD:
        ...
        for mapa_representar in mapas:
            ...
            pd.DataFrame({
                "station_id": np.arange(matrizDeseada.shape[1]),
                "t": mapa_representar,
                "value": matrizDeseada.iloc[mapa_representar, :].tolist()
            }).to_csv(join(Constantes.RUTA_SALIDA, nombrePNG.replace(".png", ".csv")), index=False)
    ...
    if mapa_circulo != Constantes.CARACTER_NULO_CMD:
        ...
        for mapa_representar in mapas:
            man_circulos = manejar_mapaCirculos(matrizDeseada, Constantes.COORDENADAS, mostrarLabels=mostrar,
            ...
            pd.DataFrame({
                "station_id": (np.arange(matrizDeseada.shape[1]) if estaciones is None else estaciones),
                "t": mapa_representar,
                "value": matrizDeseada.iloc[mapa_representar, :].tolist()
            }).to_csv(join(Constantes.RUTA_SALIDA, nombrePNG.replace(".png", ".csv")), index=False)
    ...
    if mapa_desplazamientos != Constantes.CARACTER_NULO_CMD:
        ...
        md = Manejar_Desplazamientos(matrizDeseada, Constantes.COORDENADAS, accion=int(arrayOpciones[3]),
                                     tipo=int(arrayOpciones[4]))
        ...
        # NEW CSV: summarize outgoing totals per station at t
        t = int(arrayOpciones[0])
        mat_t = matrizDeseada[t]  # expected shape [stations, stations] or similar
        mat_np = np.asarray(mat_t)
        totals = mat_np.sum(axis=1).tolist()
        pd.DataFrame({"station_id": np.arange(len(totals)), "t": t, "out_total": totals}) \
            .to_csv(join(Constantes.RUTA_SALIDA, nombrePNG.replace(".png", ".csv")), index=False)
...
def restarDirectorios(comando: [str]):
    ...
    print("Restando las matrices del directorio " + str(rutaDirectorio1) + " menos las matrices del directorio " + str(
        rutaDirectorio2))
